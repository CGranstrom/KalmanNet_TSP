...
class Plot:
    ...
    def NNPlot_Hist(self, MSE_KF_data_linear_arr, MSE_KN_linear_arr):
        ...
        plt.figure(figsize=(25, 10))
        sns.distplot(
            10 * torch.log10(MSE_KN_linear_arr),
            hist=False,
            kde=True,
            kde_kws={"linewidth": 3},
            color="g",
        ...
        sns.distplot(
            10 * torch.log10(MSE_KF_data_linear_arr),
            hist=False,
            kde=True,
            kde_kws={"linewidth": 3},
            color="r",
    ...
    def KFPlot(res_grid):
        ...
        print(
            res_grid[1][0] - res_grid[0][0],
            "[dB]",
            res_grid[2][0] - res_grid[1][0],
            "[dB]",
        )
        print(
            "KalmanNet",
        ...
        print(
            res_grid[1][1] - res_grid[0][1],
            "[dB]",
            res_grid[2][1] - res_grid[1][1],
            "[dB]",
        )
        print(
            "KalmanNet",
        ...
        print(
            res_grid[1][2] - res_grid[0][2],
            "[dB]",
            res_grid[2][2] - res_grid[1][2],
            "[dB]",
        )
        print(
            "KalmanNet",
    ...
    def NNPlot_test(
        ...
        plt.figure(figsize=(50, 20))
        sns.distplot(
            10 * torch.log10(MSE_test_linear_arr),
            hist=False,
            kde=True,
            kde_kws={"linewidth": 3},
            color="g",
        ...
        sns.distplot(
            10 * torch.log10(MSE_KF_linear_arr),
            hist=False,
            kde=True,
            kde_kws={"linewidth": 3},
            color="b",
...
class Plot_RTS(Plot):
    ...
    def NNPlot_epochs(
        ...
        N_Epochs_plt = np.floor(N_MiniBatchTrain_plt / BatchSize).astype(
            int
        )  # number of epochs
    ...
    def NNPlot_Hist(
        self, MSE_KF_linear_arr, MSE_RTS_data_linear_arr, MSE_RTSNet_linear_arr
    ):
        ...
        plt.figure(figsize=(25, 10))
        sns.distplot(
            10 * torch.log10(MSE_RTSNet_linear_arr),
            hist=False,
            kde=True,
            kde_kws={"linewidth": 5},
            color="b",
        ...
        sns.distplot(
            10 * torch.log10(MSE_RTS_data_linear_arr),
            hist=False,
            kde=True,
            kde_kws={"linewidth": 3.2, "linestyle": "--"},
            color="g",
            label="RTS Smoother",
        )
    ...
    def rotate_RTS_Plot_H(self, r, MSE_RTS_dB, rotateName):
        ...
        NoiseFloor = -x_plt
        main_H.plot(
            x_plt, NoiseFloor, "--r", linewidth=2, markersize=12, label=r"Noise Floor"
        )
        main_H.plot(
            x_plt,
            MSE_RTS_dB[0, :],
        ...
        ax2 = plt.axes([0.15, 0.15, 0.27, 0.27])
    ...
    def rotate_RTS_Plot_FHCompare(self, r, MSE_RTS_dB_F, MSE_RTS_dB_H, rotateName):
        ...
        plt.ylabel("MSE [dB]", fontsize=32)
        plt.title(
            "MSE vs inverse noise variance with inaccurate SS knowledge", fontsize=32
        )
        plt.grid(True)
        plt.savefig(fileName)
...
class Plot_extended(Plot_RTS):
    def EKFPlot_Hist(self, MSE_EKF_linear_arr):
        ...
        plt.figure(figsize=(25, 10))
        sns.distplot(
            10 * np.log10(MSE_EKF_linear_arr),
            hist=False,
            kde=True,
            kde_kws={"linewidth": 3},
            color="b",
    ...
    def KF_RTS_Plot(self, r, MSE_KF_RTS_dB):
        ...
        plt.plot(
            ...
            MSE_KF_RTS_dB[0, :],
            "-gx",
            label=r"$\mathrm{\frac{q^2}{r^2}}=0$ [dB], Toy Model, EKF",
        )
        plt.plot(
            x_plt,
            MSE_KF_RTS_dB[1, :],
            "--bo",
            label=r"$\mathrm{\frac{q^2}{r^2}}=0$ [dB], Toy Model, Extended RTS",
        )
        ...
        plt.ylabel("MSE [dB]", fontsize=32)
        plt.title(
            "Comparing Extended Kalman Filter and Extended RTS Smoother", fontsize=32
        )
        plt.xticks(fontsize=20)
        plt.yticks(fontsize=20)
        plt.grid(True)
    ...
    def NNPlot_epochs(
        ...
        N_Epochs_plt = np.floor(N_MiniBatchTrain_plt * BatchSize / N_E).astype(
            int
        )  # number of epochs
        print(N_Epochs_plt)
        # File Name
        ...
        # Train
        y_plt1 = MSE_train_dB_epoch[
            np.linspace(0, N_MiniBatchTrain_plt - 1, N_Epochs_plt)
        ]
        plt.plot(x_plt, y_plt1, KColor[0], label=ERTSlegend[0])
    ...
    def NNPlot_Hist(
        self, MSE_EKF_linear_arr, MSE_ERTS_data_linear_arr, MSE_RTSNet_linear_arr
    ):
        ...
        sns.distplot(
            10 * torch.log10(MSE_EKF_linear_arr),
            hist=False,
            kde=True,
            kde_kws={"linewidth": 3},
            color="orange",
    ...
    def NNPlot_epochs_KF_RTS(
        self,
        N_MiniBatchTrain_plt,
        BatchSize,
        MSE_EKF_dB_avg,
        MSE_ERTS_dB_avg,
        MSE_KNet_test_dB_avg,
        MSE_KNet_cv_dB_epoch,
        MSE_KNet_train_dB_epoch,
        MSE_RTSNet_test_dB_avg,
        MSE_RTSNet_cv_dB_epoch,
    ...
    def plotTrajectories(self, inputs, dim, titles, file_name):
        ...
        for title in titles:
            ...
            if dim == 2:
                ...
                ax.plot(
                    inputs_numpy[0, 0, :], inputs_numpy[0, 1, :], "b", linewidth=0.75
                )
                ax.set_xlabel("x1")
                ax.set_ylabel("x2")
                ax.set_title(
            ...
            if dim == 4:
                ...
                ax.plot(
                    np.arange(np.size(inputs_numpy[0, :], axis=1)),
                    inputs_numpy[0, 0, :],
                    "b",
                    linewidth=0.75,
                )
                if title != "True Trajectory":
                    ...
                    for peak, trough in zip(peaks, troughs):
                        plt.axvspan(peak, trough, color="red", alpha=0.2)
                # zoomed in
    ...
    def Partial_Plot_Lor(self, r, MSE_Partial_dB):
        ...
        NoiseFloor = -x_plt
        main_partial.plot(
            x_plt, NoiseFloor, "--r", linewidth=3, markersize=12, label=r"Noise Floor"
        )
        main_partial.plot(
            x_plt,
            MSE_Partial_dB[0, :],
        ...
        ax2 = plt.axes([0.15, 0.15, 0.25, 0.25])
